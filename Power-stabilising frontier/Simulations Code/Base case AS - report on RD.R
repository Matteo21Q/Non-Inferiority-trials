...
for (j in 1:n.pi0) {
  ...
  for (i in 1:n.sim) {
    ...
    NI.marg.ASfront<-sin(asin(sqrt(pio0))+asin(sqrt(pif1))-asin(sqrt(pie0)))^2-pio0

    p.value<-pnorm((AS-NI.marg)/seAS)
    RD<-simulations[i,2,j]/sample.size[2]-simulations[i,1,j]/sample.size[1]
    seRD<-sqrt((simulations[i,2,j]/sample.size[2])*(1-simulations[i,2,j]/sample.size[2])/sample.size[2]+(simulations[i,1,j]/sample.size[1])*(1-simulations[i,1,j]/sample.size[1])/sample.size[1])
    p.value.RD<-pnorm((RD-NI.marg.RD)/seRD)
    simulations[i,7,j]<-RD-seRD*qnorm(p.value)
    SE.RDeq<-(RD-NI.marg.ASfront)/qnorm(p.value)
    simulations[i,8,j]<-pnorm(qnorm(0.975)*SE.RDeq/seRD)
...
yAS<-sin(asin(sqrt(range.of.pi0))+asin(sqrt(pif1))-asin(sqrt(pie0)))^2-range.of.pi0

# save results

# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Base case AS - Report on RD.RData")

# Plot


par(mar=c(5.1, 4.1, 4.1, 6.1), xpd=FALSE)
plot(range.of.pi0, mean.margin, type = "b", pch=16, ylim=c(0,0.15), xlab = expression(pi[0]), ylab= "NI margin", main = "Equivalent NI Margin")
