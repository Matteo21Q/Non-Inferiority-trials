...
for (j in 1:n.pi0) {
  ...
  sample.size<-sam_size_AS(alph, 1-power, pie0, pie1, NI.marg,r)  # Arc-sine
  #sample.size<-sam_size_RD(alph, 1-power, pie0, pie1, NI.marg.RD,r)  # Risk Difference
  #sample.size<-sam_size_RR(alph, 1-power, pie0, pie1, NI.marg.RR,r)  # Risk Ratio
...
plot(range.of.pi0, T1Er.method[,1], type = "b", pch=16, ylim=c(0,0.2), xlab = expression(pi[0]), ylab= "Type 1 Error Rate", main = "AsinDiff")
