...
Power.method<-matrix(NA,n.pi0,5)
colnames(Power.method)<-1:5
...
for (j in 1:n.pi0) {
  ...
  for (i in 1:n.sim) {
    ...
    simulations[i,5,j]<-ci.pd(matrix(c(simulations[i,1:2,j], simulations[i,3:4,j]-simulations[i,1:2,j]),ncol=2, byrow=TRUE), alpha = alph*2, print=F)[6]
  ...
  Power.method[j,4]<-sum(simulations[,11,j])/n.sim
  Power.method[j,5]<-sum(simulations[,13,j])/n.sim
...
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Base case RD Adjust alpha - Power.RData")
...
lines(range.of.pi0, Power.method[,4],  col="purple", type = "b", pch=16)
lines(range.of.pi0, Power.method[,5],  col="brown", type = "b", pch=16)
abline(h=0.9, col="red")
par(xpd=TRUE)
legend("bottomleft",inset=c(-0.35,0), legend = c("Don't modify", "Modify margin but not alpha", "Modify margin alpha=0.025", "Modify margin alpha=0.03", "Modify margin alpha=0.04"), pch=16, col = c("black","orange", "red", "purple", "brown")) # optional legend
par(xpd=FALSE)
