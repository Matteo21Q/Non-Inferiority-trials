...
Power.method<-matrix(NA,n.pi0,4)
colnames(Power.method)<-1:4
...
for (j in 1:n.pi0) {
  ...
  for (i in 1:n.sim) {
    ...
    NI.marg2<-ifelse(abs(log(pie0/pio0))>log(1.5),
                     log(sin(asin(sqrt(pio0))+asin(sqrt(pif1))-asin(sqrt(pie0)))^2/pio0),
                     NI.marg)
    simulations[i,7,j]<-(log(simulations[i,5,j])<NI.marg2)
    NI.marg3<-ifelse(abs(log(pie0/pio0))>log(1.25),
                     log(sin(asin(sqrt(pio0))+asin(sqrt(pif1))-asin(sqrt(pie0)))^2/pio0),
                     NI.marg)
    simulations[i,8,j]<-(log(simulations[i,5,j])<NI.marg3)
    NI.marg4<-ifelse(abs(log(pie0/pio0))>log(2),
                     log(sin(asin(sqrt(pio0))+asin(sqrt(pif1))-asin(sqrt(pie0)))^2/pio0),
                     NI.marg)
    simulations[i,9,j]<-(log(simulations[i,5,j])<NI.marg4)
    if (is.na(simulations[i,6,j])) simulations[i,6:9,j]<-0 # Too little events, hence equivalent to not being able to prove non-inferiority
  ...
  Power.method[j,3]<-sum(simulations[,8,j])/n.sim
  Power.method[j,4]<-sum(simulations[,9,j])/n.sim
...
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Base case RR - Power.RData")
...
legend("bottomright", legend = c("Method 1", "Method 2", "Method 3", "Method 4"), pch=16, col = c("black","blue","orange", "green")) # optional legend
