...
simulations<-array(NA, c(n.sim,13, n.pi0))
T1Er.method<-matrix(NA,n.pi0,5)
colnames(T1Er.method)<-1:5
...
for (j in 1:n.pi0) {
  ...
  for (i in 1:n.sim) {
    ...
    simulations[i,8,j]<-ci.pd(matrix(c(simulations[i,1:2,j], simulations[i,3:4,j]-simulations[i,1:2,j]),ncol=2, byrow=TRUE), alpha = 0.02*2, print=F)[6]
    simulations[i,9,j]<-(abs(simulations[i,8,j])<NI.marg2)
    simulations[i,10,j]<-ci.pd(matrix(c(simulations[i,1:2,j], simulations[i,3:4,j]-simulations[i,1:2,j]),ncol=2, byrow=TRUE), alpha = 0.015*2, print=F)[6]
    simulations[i,11,j]<-(abs(simulations[i,10,j])<NI.marg2)
    simulations[i,12,j]<-ci.pd(matrix(c(simulations[i,1:2,j], simulations[i,3:4,j]-simulations[i,1:2,j]),ncol=2, byrow=TRUE), alpha = 0.01*2, print=F)[6]
    simulations[i,13,j]<-(abs(simulations[i,12,j])<NI.marg2)
    
  }
  
  T1Er.method[j,1]<-sum(simulations[,6,j])/n.sim
  T1Er.method[j,2]<-sum(simulations[,7,j])/n.sim
  T1Er.method[j,3]<-sum(simulations[,9,j])/n.sim
  T1Er.method[j,4]<-sum(simulations[,11,j])/n.sim
  T1Er.method[j,5]<-sum(simulations[,13,j])/n.sim
...
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Base case RD Adjust alpha - T1Er.RData")
...
lines(range.of.pi0, T1Er.method[,2],  col="orange", type = "b", pch=16)
lines(range.of.pi0, T1Er.method[,3], col="red", type = "b", pch=16)
lines(range.of.pi0, T1Er.method[,4],  col="purple", type = "b", pch=16)
lines(range.of.pi0, T1Er.method[,5],  col="brown", type = "b", pch=16)
abline(h=0.05, col="red")
par(xpd=TRUE)
legend("topright",inset=c(-0.35,0), legend = c("Don't modify", "Modify margin but not alpha", "Modify margin alpha=0.05", "Modify margin alpha=0.06", "Modify margin alpha=0.08"), pch=16, col = c("black","orange", "red", "purple", "brown")) # optional legend
par(xpd=FALSE)
