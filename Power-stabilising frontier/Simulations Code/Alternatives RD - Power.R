...
alt3.par$NI.marg<-alt3.par$pif1-alt3.par$pie0
alt4.par$pif1<-0.15                        # Fourth alternative, happy to tolerate up to 15%
alt4.par$NI.marg<-alt4.par$pif1-alt4.par$pie0
...
range.of.pi0<-seq(0.005,0.20,0.005)
n.pi0<-length(range.of.pi0)
simulations<-array(NA, c(n.sim,42, n.pi0))
Power.method<-matrix(NA,n.pi0,7)
colnames(Power.method)<-1:7
...
for (a in 1:n.alt) {
  ...
  for (j in 1:n.pi0) {
    ...
    pi1<-ifelse(a==2,pi0/2,pi0)  # True active event rate
    ...
    for (i in 1:n.sim) {
      ...
      simulations[i,5+(a-1)*6,j]<-ci.pd(matrix(c(simulations[i,1:2+(a-1)*6,j], simulations[i,3:4+(a-1)*6,j]-simulations[i,1:2+(a-1)*6,j]),ncol=2, byrow=TRUE), alpha = alph*2, print=F)[6]
      ...
      NI.marg2<-ifelse(abs(pie0-pio0)>0.025,
                       ...
                       # NI.marg2<-ifelse(abs(pie0-pio0)>0.05,
                       sin(asin(sqrt(pio0))+asin(sqrt(pif1))-asin(sqrt(pie0)))^2-pio0,
                       NI.marg)
      simulations[i,6+(a-1)*6,j]<-(abs(simulations[i,5+(a-1)*6,j])<NI.marg2)
      
    }
    
    Power.method[j,a]<-sum(simulations[,6+(a-1)*6,j])/n.sim
...
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Alternatives RD - Power.RData")
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Alternatives 3 RD - Power.RData")
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Alternatives 4 RD - Power.RData")
...
lines(range.of.pi0, Power.method[,2],  col="blue", type = "b", pch=16)
lines(range.of.pi0, Power.method[,3], col="orange", type = "b", pch=16)
lines(range.of.pi0, Power.method[,4],  col="green", type = "b", pch=16, lty=2)
lines(range.of.pi0, Power.method[,5],  col="red", type = "b", pch=16, lty=2)
lines(range.of.pi0, Power.method[,6],  col="violet", type = "b", pch=16, lty=2)
lines(range.of.pi0, Power.method[,7],  col="grey", type = "b", pch=16, lty=2)
par(xpd=TRUE)
