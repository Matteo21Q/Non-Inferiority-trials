...
for (a in 1:n.alt) {
  ...
  for (j in 1:n.pi0) {
    ...
    Power.method[j,a]<-sum(simulations[,6+(a-1)*6,j], na.rm=T)/(n.sim-sum(is.na(simulations[,6+(a-1)*6,j])))
...
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Alternatives RR - Power.RData")
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Alternatives 3 RR - Power.RData")
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Alternatives 4 RR - Power.RData")

# plot

par(mar=c(5.1, 4.1, 4.1, 8.1), xpd=FALSE)
plot(range.of.pi0, Power.method[,1], type = "b", pch=16, ylim=c(0,1), xlab = expression(pi[0]), ylab= "Power", main = "Risk ratio")
