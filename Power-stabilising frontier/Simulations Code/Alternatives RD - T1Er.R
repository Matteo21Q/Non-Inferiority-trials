...
for (a in 1:n.alt) {
  ...
  for (j in 1:n.pi0) {
    ...
    #pi1<-ifelse(a==3,pi0+0.025,ifelse(a==4,pi0+0.1,pi0+0.05))  # True active event rate
    ...
    for (i in 1:n.sim) {
      ...
      simulations[i,5+(a-1)*6,j]<-ci.pd(matrix(c(simulations[i,1:2+(a-1)*6,j], simulations[i,3:4+(a-1)*6,j]-simulations[i,1:2+(a-1)*6,j]),ncol=2, byrow=TRUE), alpha = 2*alph, print=F)[6]
    ...
    T1Er.method[j,a]<-sum(simulations[,6+(a-1)*6,j])/n.sim
...
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Alternatives RD - T1Er.RData")
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Alternatives 3 RD - T1Er.RData")
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Alternatives 4 RD - T1Er.RData")
# save.image("~/My Documents/Non-inferiority trials/NI Frontiers/Simulation results/Alternatives RD - T1Er DGM Ian.RData")
